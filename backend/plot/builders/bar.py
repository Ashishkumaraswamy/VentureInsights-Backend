import plotly.express as px
from backend.plot.types import ChartData
from . import IBuilder
import tempfile


class BarBuilder(IBuilder):
    async def plot(self, chart_data: ChartData, company_name: str) -> str:
        fig = self._build_bar_plot(
            chart_data.data, title=chart_data.title, x=chart_data.x, y=chart_data.y
        )
        # Save to a temporary HTML file
        with tempfile.NamedTemporaryFile(delete=False, suffix=".html") as tmp:
            fig.write_html(tmp.name, include_plotlyjs="cdn", full_html=True)
            tmp_path = tmp.name
        url = await self.netlify_agent.upload_html(tmp_path)
        return url

    @staticmethod
    def _build_bar_plot(data, title=None, x=None, y=None, color=None, **kwargs):
        fig = px.bar(
            data,
            x=x,
            y=y,
            color=color,
            title=title,
            template="plotly_white",
            color_discrete_sequence=px.colors.qualitative.Vivid,
            **kwargs,
        )
        # Rounded bars and bar width
        for trace in fig.data:
            trace.marker.line.width = 1.5
            trace.marker.line.color = "#fff"
            trace.marker.opacity = 0.92
        fig.update_traces(
            marker_line_width=1.5, marker_line_color="#fff", marker_opacity=0.92
        )
        fig.update_layout(
            title_font_size=22,
            title_x=0.5,
            font=dict(family="Inter, Arial", size=15, color="#222"),
            legend=dict(
                orientation="h",
                yanchor="bottom",
                y=1.02,
                xanchor="right",
                x=1,
                bgcolor="rgba(255,255,255,0.7)",
                bordercolor="#E2E2E2",
                borderwidth=1,
            ),
            xaxis=dict(
                showgrid=True,
                gridcolor="#F0F0F0",
                title=dict(text=x if x else "", font=dict(size=16)),
                zeroline=False,
            ),
            yaxis=dict(
                showgrid=True,
                gridcolor="#F0F0F0",
                title=dict(text=y if y else "", font=dict(size=16)),
                zeroline=False,
            ),
            hovermode="x unified",
            margin=dict(l=40, r=40, t=60, b=40),
            bargap=0.18,
            plot_bgcolor="#fff",
            paper_bgcolor="#fff",
        )
        return fig

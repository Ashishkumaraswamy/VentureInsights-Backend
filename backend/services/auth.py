...
class AuthService:
    ...
    async def signup(self, signup_request:SignUpRequest):
        ...
        try:
            ...
            raise ServiceException(status=Status.EXECUTION_ERROR, message=f"Failed to create user due to {e}")

    async def login(self, login_request: LoginRequest):

        user_details = await self.mongo_connector.aquery("users", {"email": login_request.email})
        ...
        if bcrypt.checkpw(raw_password.encode('utf-8'), hashed_password.encode('utf-8')):

...
class ChatService:
    ...
    def _create_indexes(self):
        ...
        message_indexes = [
            ...
            MongoIndexSpec(keys=[("thread_id", 1), ("timestamp", 1)], name="thread_id_timestamp"),
    ...
    async def get_threads(self, limit: int = 10, offset: int = 0, user_id: Optional[str] = None) -> tuple[List[ChatThread], int]:
    ...
    async def delete_thread(self, thread_id: str) -> bool:
        ...
        thread_collection = await self.mongo_connector.aget_collection(self.THREADS_COLLECTION)
        await thread_collection.delete_one({"_id": thread_id})
        
        # Delete all messages for this thread
        message_collection = await self.mongo_connector.aget_collection(self.MESSAGES_COLLECTION)
    ...
    async def add_message(self, thread_id: str, message: SendMessageRequest) -> MessageResponse:
        ...
        if len(message_docs) > 1:
            response_content += f" I see our conversation has {len(message_docs)} messages so far."
